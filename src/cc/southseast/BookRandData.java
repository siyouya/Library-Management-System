...
public class BookRandData {
    ...
    public static void main(String args[]) {
        ...
        for (int i = 1; i <= 100; i++) {
            ...
            writer += firstName.charAt(Math.abs(rand.nextInt()) % firstName.length());

            for (int j = 0; j < nameLength; j++){
                if (sex.equals("女"))
                    writer += girl.charAt(Math.abs(rand.nextInt()) % girl.length());
                else
                    writer += boy.charAt(Math.abs(rand.nextInt()) % boy.length());

            }
            book.setWriter(writer);

            book.setBookName(writer+writer);

            book.setBookPrice(rand.nextInt(10)+20);


            String publishId = "";

            for (int j = 0; j < 10; j++) {
                publishId += rand.nextInt(10);
            }

            book.setPublishId(Long.parseLong(publishId));
