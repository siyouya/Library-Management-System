...
public class UserRandData {
    ...
    public static void main(String args[]) {
        ...
        for (int i = 1; i <= 100; i++) {
            ...
            name += firstName.charAt(Math.abs(rand.nextInt()) % firstName.length());

            for (int j = 0; j < nameLength; j++){
                if (user.getSex().equals("女"))
                    name += girl.charAt(Math.abs(rand.nextInt()) % girl.length());
                else
                    name += boy.charAt(Math.abs(rand.nextInt()) % boy.length());
